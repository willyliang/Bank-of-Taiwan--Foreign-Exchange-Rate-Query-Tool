...
function Update-AllLabels {
    ...
    if ($null -ne $script:Chart) {
        ...
        # Update legend CustomItem text (handles hidden/shown state colors)
        Update-LegendText -Chart $script:Chart
...
# Clicking a CustomItem legend entry hides/shows the corresponding series.
# Hidden series: line becomes invisible (transparent), legend item dims.
# Shown series: line in original color, legend item bright.
# The legend item is ALWAYS visible and clickable (never disappears).
$script:Chart.Add_MouseClick({
    param($sender, $e)

    if ($script:IsClosing) { return }

    $chartControl = $sender
    $result = $chartControl.HitTest($e.X, $e.Y)

    # Only handle clicks on Legend items (CustomItems or auto-generated)
    if ($null -ne $result -and
        $result.ChartElementType -eq [System.Windows.Forms.DataVisualization.Charting.ChartElementType]::LegendItem) {

        $legendItem = $result.Object
        if ($null -ne $legendItem) {
            # CustomItem: Name is the series name we set in New-RateChart
            # Auto-generated: SeriesName is the series name
            $seriesName = $legendItem.Name
            if ([string]::IsNullOrWhiteSpace($seriesName) -or $null -eq $chartControl.Series[$seriesName]) {
                $seriesName = $legendItem.SeriesName
            }

            if (-not [string]::IsNullOrWhiteSpace($seriesName) -and $null -ne $chartControl.Series[$seriesName]) {
                Switch-ChartSeries -Chart $chartControl -SeriesName $seriesName | Out-Null
